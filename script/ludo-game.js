...
function nextPlayer() {
  ...
  if (playerNo == noOfPlayer) playerNo = 0;
  ...
  // Skip players that have already won or don't exist in the current game
  if (playerNo > 0 && playerNo <= noOfPlayer) {
    if (
      ...
      $(".player-card").removeClass("player-card-active");
      $("#player" + playerNo + "Card").addClass("player-card-active").css("display", "block");
    }
  } else {
    nextPlayer();
  }
...
function cutPawn(noInId, moveingClassValue) {
  if (players[playerName].outArea.includes(moveingClassValue)) {
    // Safe squares where pawns can't be cut
    if ([1, 9, 14, 22, 27, 35, 40, 48].includes(noInId)) {
      return;
    } else {
      let colorInClass = getColorFromValue(moveingClassValue);
      let targetClass = null;
      
      // Check if there are any pawns to cut at this position
      for (const cValve of outAreaPos[noInId]) {
        if (colorInClass != getColorFromValue(cValve)) {
          targetClass = cValve;
          break; // Only cut one pawn at a time
        }
      }
      ...
      if (targetClass != null) {
        ...
        let cutColor = getColorFromValue(targetClass);
        let pName = cutColor + "Player";
...
/* start game */
function startGame() {
  // Show player cards based on number of players
  $(".player-card").css("display", "none");
  
  if (noOfPlayer >= 1) {
    $("#player1Card").css("display", "block");
  }
  if (noOfPlayer >= 2) {
    $("#player2Card").css("display", "block");
  }
  if (noOfPlayer >= 3) {
    $("#player3Card").css("display", "block");
  }
  if (noOfPlayer >= 4) {
    $("#player4Card").css("display", "block");
  }
...
function restartGame() {
  ...
  $(".badge-box").remove();
  $(".player-card").css("display", "none").removeClass("player-card-active");
